...
router.get('/:shortUrl',async(req,res)=>{
    ...
    try{
        if(url){
            ...
            if(diff>48){
                url.delete()
                res.render('dashboard', { logged: true, csrfToken: req.csrfToken(), err: "The URL is invalid; please generate a new one." })
            }
             return res.redirect(url.longUrl)
            
            
        }else{
            res.render('dashboard', { logged: true, csrfToken: req.csrfToken(), err: "The shortened URL is not present in our system." })
        }
